      ...
      <DataTable>
        ...
        <template v-slot:tb-btn-tab>
            <div class="btn-group mb-1" role="group">
              ...
              <!-- <button type="button" :class="{'btn':true ,'btn-primary': isActive != 2, 'active-nav': isActive == 2}" @click="filterStatus(2, 'tagged by AP tagging')">Pending</button> -->
              <button type="button" :class="{'btn':true ,'btn-primary': isActive != 3, 'active-nav': isActive == 3}" @click="filterStatus(3, 'hold by AP tagging')">Released</button>
              <button type="button" :class="{'btn':true ,'btn-primary': isActive != 4, 'active-nav': isActive == 4}" @click="filterStatus(4, 'rejected by AP tagging')">Returned</button>
...
export default {
    ...
    created(){
      ...
      this.$store.dispatch('setHeaderTitle','Releasing of Cheque')
    },
    computed:{
      ...mapState([
        'currentPage',
    ...
    methods:{
      ...mapActions('requests', ['getAllTransaction', 'modifySingleTransaction']),
