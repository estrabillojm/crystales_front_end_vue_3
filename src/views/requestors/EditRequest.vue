    ...
    <Navbar/>
        ...
        <div class="grid-header">
            <div class="row pt-2 pl-4 pr-4"> 
                        ...
                        <div class="row">
                          <div class="col-lg-12" v-if="documentType == 9 || documentType == 6">
                            <div class="form-group">
                              ...
                              <input type="date" :class="{dateError: dateError, 'form-control': true}" v-model="dateFrom">
                              <span class="text-danger date-error" v-if="dateError">This date must not be exceeded to the Date Field below*</span>
                            </div>
                          </div>
                        ...
                        <div class="row">
                          ...
                          <div class="col-lg-12" v-if="documentType == 7 || documentType == 2 || documentType == 4 || documentType == 5 || documentType == 9 || documentType == 6 || documentType == 8">
                            <div class="form-group">
                              ...
                              <input type="text" class="form-control disable" disabled v-if="documentType == 5" v-model="documentAmount" @keyup="addCommaDocAmount">
                              <input type="text" class="form-control" v-else v-model="documentAmount" @keyup="addCommaDocAmount">
                            </div>
                          </div>
                          
                          <div class="col-lg-12" v-if="documentType">
                            <div class="form-group">
                    ...
                    <div class="row" v-if="documentType == 8">
                          ...
                          <div class="col-lg-6 p-1">
                            <div class="form-group">
                              ...
                              <input type="date" class="form-control" v-model="pcfDate">
                            </div>
                          </div>
                          ...
                          <div class="col-lg-6 p-1">
                            <div class="form-group">
                              ...
                              <select class="form-control" v-model="pcfLetter">
                                <option v-for="(letter, index) in letters" :key="index" :value="letter">{{ letter.toUpperCase() }}</option>
                              </select>
                ...
                <!-- ADD REF -->
                <div class="col-lg-6 p-1" v-if="(documentType == 4 || documentType == 5)">
                  <div class="col-lg-12 px-5 pb-5 my-card">
                      <!-- <div class="absolute-div" v-if="documentType == 7"><em>Optional</em></div> -->
                        ...
                        <div class="row">
                          ...
                          <div class="col-lg-12 px-1 mt-0" v-if="supplierType=='si' || supplierType == 'pr'">
                            <div class="form-group">
...
export default {
    ...
    methods:{
      ...
      submitRef(){
        ...
        }else if(this.supplierType == 'si' || this.supplierType == 'pr'){
          if(this.refAmount || this.refQty){
            this.refList.unshift({
              referrence_type: this.supplierType,
              referrence_no: this.refNumber,
    ...
    watch:{
      ...
      async documentCategory(){
        ...
        if(this.documentCategory){
          ...
          await axios.get(`/categories/${this.documentCategory}`).then(res=>{
              let holder = res.data
              let result = holder
      ...
      async documentType(){
        ...
        if(this.documentType){
          ...
          await axios.get(`/documents/${this.documentType}`).then(res=>{
              let holder = res.data
              let result = holder[0]
              
              output = result.document_type
          })
          this.documentTypeInfo = output
          // console.log('Company Info',this.companyInfo)
        }
      },
      async documentSupplier(){
        let output
        if(this.documentSupplier){
    
          await axios.get(`/suppliers/${this.documentSupplier}`).then(res=>{
              let holder = res.data
              let result = holder.data
              
              output = result[0].supplier_name
          })
          this.supplierInfo = output
          // console.log('Company Info',this.companyInfo)
        }
      },
      async documentCompany(){
        let output
        if(this.documentCompany){
    
          await axios.get(`/companies/${this.documentCompany}`).then(res=>{
              let holder = res.data
              let result = holder.data
              output = `${result.company_code}-${result.company_description}`
          })
          this.companyInfo = output
          // console.log('Company Info',this.companyInfo)
        }
      }
    },
    computed:{
      ...
      totalBalanceOne(){
        ...
        if(this.totalRefAmount && this.documentAmount){
          let documentAmount = this.documentAmount.replace(/,/g, '')
          let totalRefAmount = this.totalRefAmount.replace(/,/g, '')
          if(this.documentType == 4){ // CONTRACTOR'S BILLING
            let total = parseFloat(documentAmount) - parseFloat(totalRefAmount)
            holder = total.toString()
          }
        }


        if(this.totalRefAmount && this.totalAmount){
        ...
        if(this.documentAmount){
          let documentAmount = this.documentAmount.replace(/,/g, '')

          if(this.documentType == 9 || this.documentType == 8 || this.documentType == 6){ //UTILITIES || PCF
            let total = parseFloat(documentAmount)
            holder = total.toString()
          }
        }
      ...
      emptyFieldValidation(){
        ...
        if(this.documentType == 2 || this.documentType == 4){ // PRM , CONTRACTORS BILLING
          if(this.documentCategory &&
            this.paymentType &&
            this.documentCompany &&
            this.documentNumber &&
        ...
        if( this.documentType == 5){ // RECEIPT
          if(this.documentCategory &&
            this.paymentType &&
            this.documentCompany &&
            this.documentNumber &&
        ...
        if( this.documentType == 6 || this.documentType == 9){ // PAYROLL
          if(this.paymentType &&
            this.documentAmount &&
            this.documentCompany &&
            this.documentSupplier &&
      ...
      documentValidation(){
        ...
        if(this.documentType){
          ...
          if(this.documentType == 2 && this.documentAmount){
            // console.log("VALIDATION", this.emptyFieldValidation ,this.totalAmount)
          ...
          // Receipt Validation
            if(this.documentType == 5){
              ...
              if(parseFloat(totalRefAmount) == parseFloat(totalPoAmount) && parseFloat(totalPoAmount) && parseFloat(totalRefAmount)){
                return true
              }else{
                return false
              }
          ...
          // Payroll and Utilities Validation
          if((this.documentType == 6 || this.documentType == 9) && this.emptyFieldValidation){
          ...
          // PCF validation
          if(this.documentType == 8 && this.emptyFieldValidation){
            if(this.pcfDate && this.pcfLetter){
              let check = moment(this.pcfDate, 'YYYY/MM/DD')
              let month = check.format('MMMM')
              let year = check.format('YYYY')
              this.pcfDateConvert = `${month} ${year}`
              return true
            }else{
              return false
            }
